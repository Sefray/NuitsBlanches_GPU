...
void BM_Threshold(benchmark::State& state)
{
  ...
  // get_connected_components(d_buffer_A, d_buffer_B, width, height, minimum_pixel);
...
void BM_Connectic_components(benchmark::State& state)
{
  ...
  for (auto _ : state)
  {
    ...
    gpu::one::two::three::four::get_connected_components(d_buffer_C, d_buffer_B, width, height, minimum_pixel);
  }
