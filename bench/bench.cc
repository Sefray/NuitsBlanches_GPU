...
void BM_Detection_gpu(benchmark::State& st, std::string ref_filename, std::string modified_filename,
                      enum mode_cc mode_cc)
{
  png::image<png::rgb_pixel> ref(ref_filename);
  png::image<png::rgb_pixel> modified(modified_filename);
  ...
  for (auto _ : st)
    auto components = gpu::pipeline(d_ref_smoothed, modified.get_pixbuf(), width, height, kernel_size,
                                    kernel_size_opening, kernel_size_closing, binary_threshold, mode_cc, minimum_pixel);
...
BENCHMARK_CAPTURE(BM_Detection_gpu, scia_premium_first, std::string("../data/scia_premium_0001.png"),
                  std::string("../data/scia_premium_0002.png"), union_find);
